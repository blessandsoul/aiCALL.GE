...
const { verifyContactChallenge } = load('../src/lib/contact-challenge.ts', { '@/config/contact-challenge': { CONTACT_CHALLENGE_HOSTS: ['aicall.ge', 'www.aicall.ge'] } });

test('contact challenge rejects bypasses and only delivers a verified request', async () => {
  const originalFetch = globalThis.fetch;
  const originalSecret = process.env.TURNSTILE_SECRET_KEY;
  let calls = 0;
  let result = { success: true, action: 'contact', hostname: 'aicall.ge' };
  ...
  try {
    ...
    for (const invalid of [
      ...
      { success: true, action: 'login', hostname: 'aicall.ge' },
    ...
    const request = (body) => new Request('https://aicall.ge/api/contact', {
    ...
    assert.equal(sent, 0);
    globalThis.fetch = async () => Response.json({ success: true, action: 'contact', hostname: 'aicall.ge' });
